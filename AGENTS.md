# Scalify Printed CLI Agent Guide

This directory is a generated `scalify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
scalify-pp-cli doctor --json
scalify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
scalify-pp-cli which "<capability>" --json
scalify-pp-cli <command> --help
```
...
```bash
scalify-pp-cli <command> --help
scalify-pp-cli <command> --dry-run --agent
```
