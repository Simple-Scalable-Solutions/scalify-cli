...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".scalify-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCALIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCALIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "scalify-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.scalify-pp-cli/feedback.jsonl.
When ` + "`SCALIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCALIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scalify-pp-cli feedback list
  scalify-pp-cli feedback list --limit 5
  scalify-pp-cli feedback list --json`,
