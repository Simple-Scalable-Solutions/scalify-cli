...
func newSocialMediaPostingGetSocialMediaPostingCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/social-media-posting/{id}"
			path = replacePathParam(path, "id", args[0])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "social-media-posting", false, path, params, nil)
			if err != nil {
